...
//
//	client.Plan.Query().
//		GroupBy(plan.FieldSubscription).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *PlanQuery) loadTenants(ctx context.Context, query *TenantQuery, nodes []*Plan, init func(*Plan), assign func(*Plan, *Tenant)) error {
	...
	for _, n := range neighbors {
		fk := n.plan_tenants
		if fk == nil {
			return fmt.Errorf(`foreign-key "plan_tenants" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "plan_tenants" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
