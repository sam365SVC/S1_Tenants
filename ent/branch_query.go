...
func (_q *BranchQuery) QueryTenant() *TenantQuery {
...
func (_q *BranchQuery) QueryEmployees() *EmployeeQuery {
...
func (_q *BranchQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *BranchQuery) FirstIDX(ctx context.Context) int {
...
func (_q *BranchQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *BranchQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *BranchQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *BranchQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *BranchQuery) WithTenant(opts ...func(*TenantQuery)) *BranchQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *BranchQuery) WithEmployees(opts ...func(*EmployeeQuery)) *BranchQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *BranchQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Branch, error) {
	...
	if query := _q.withTenant; query != nil {
		if err := _q.loadTenant(ctx, query, nodes, nil,
			func(n *Branch, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *BranchQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Branch, init func(*Branch), assign func(*Branch, *Tenant)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Branch)
	for i := range nodes {
		if nodes[i].tenant_branches == nil {
			continue
		}
		fk := *nodes[i].tenant_branches
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_branches" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *BranchQuery) loadEmployees(ctx context.Context, query *EmployeeQuery, nodes []*Branch, init func(*Branch), assign func(*Branch, *Employee)) error {
	...
	query.Where(predicate.Employee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(branch.EmployeesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.branch_employees
		if fk == nil {
			return fmt.Errorf(`foreign-key "branch_employees" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
