...
type InvitationQuery struct {
	...
	withInvitationEmployee *InvitationEmployeeQuery
...
// QueryInvitationEmployee chains the current query on the "invitation_employee" edge.
func (_q *InvitationQuery) QueryInvitationEmployee() *InvitationEmployeeQuery {
...
func (_q *InvitationQuery) Clone() *InvitationQuery {
	...
	return &InvitationQuery{
		...
		withInvitationEmployee: _q.withInvitationEmployee.Clone(),
...
// WithInvitationEmployee tells the query-builder to eager-load the nodes that are connected to
// the "invitation_employee" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *InvitationQuery) WithInvitationEmployee(opts ...func(*InvitationEmployeeQuery)) *InvitationQuery {
	query := (&InvitationEmployeeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withInvitationEmployee = query
	return _q
}
...
func (_q *InvitationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invitation, error) {
	...
	if query := _q.withInvitationEmployee; query != nil {
		if err := _q.loadInvitationEmployee(ctx, query, nodes, nil,
			func(n *Invitation, e *InvitationEmployee) { n.Edges.InvitationEmployee = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *InvitationQuery) loadInvitationEmployee(ctx context.Context, query *InvitationEmployeeQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *InvitationEmployee)) error {
	...
	query.Where(predicate.InvitationEmployee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(invitation.InvitationEmployeeColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.invitation_invitation_employee
		if fk == nil {
			return fmt.Errorf(`foreign-key "invitation_invitation_employee" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "invitation_invitation_employee" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
