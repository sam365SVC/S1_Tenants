...
func (_q *EmailQuery) QueryUser() *UserQuery {
...
func (_q *EmailQuery) QueryEmployees() *EmployeeQuery {
...
func (_q *EmailQuery) WithUser(opts ...func(*UserQuery)) *EmailQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *EmailQuery) WithEmployees(opts ...func(*EmployeeQuery)) *EmailQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EmailQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Email, error) {
	...
	if query := _q.withEmployees; query != nil {
		if err := _q.loadEmployees(ctx, query, nodes,
			func(n *Email) { n.Edges.Employees = []*Employee{} },
			func(n *Email, e *Employee) { n.Edges.Employees = append(n.Edges.Employees, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EmailQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Email, init func(*Email), assign func(*Email, *User)) error {
...
func (_q *EmailQuery) loadEmployees(ctx context.Context, query *EmployeeQuery, nodes []*Email, init func(*Email), assign func(*Email, *Employee)) error {
	...
	query.Where(predicate.Employee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(email.EmployeesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.email_employees
		if fk == nil {
			return fmt.Errorf(`foreign-key "email_employees" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "email_employees" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
