...
func (_q *UserQuery) QueryEmployees() *EmployeeQuery {
...
func (_q *UserQuery) WithEmployees(opts ...func(*EmployeeQuery)) *UserQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withEmployees; query != nil {
		if err := _q.loadEmployees(ctx, query, nodes,
			func(n *User) { n.Edges.Employees = []*Employee{} },
			func(n *User, e *Employee) { n.Edges.Employees = append(n.Edges.Employees, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadEmployees(ctx context.Context, query *EmployeeQuery, nodes []*User, init func(*User), assign func(*User, *Employee)) error {
	...
	query.Where(predicate.Employee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.EmployeesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_employees
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_employees" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_employees" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
