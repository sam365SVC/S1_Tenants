...
func (_q *UserQuery) QueryEmails() *EmailQuery {
...
func (_q *UserQuery) QueryOrganization() *TenantQuery {
...
func (_q *UserQuery) WithEmails(opts ...func(*EmailQuery)) *UserQuery {
	query := (&EmailClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithOrganization(opts ...func(*TenantQuery)) *UserQuery {
	query := (&TenantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withOrganization; query != nil {
		if err := _q.loadOrganization(ctx, query, nodes, nil,
			func(n *User, e *Tenant) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadEmails(ctx context.Context, query *EmailQuery, nodes []*User, init func(*User), assign func(*User, *Email)) error {
...
func (_q *UserQuery) loadOrganization(ctx context.Context, query *TenantQuery, nodes []*User, init func(*User), assign func(*User, *Tenant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
